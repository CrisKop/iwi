...
exports.run = async(client, message, args, Discord) =>{
    if(!message.member.hasPermission("ADMINISTRATOR")) return message.channel.send("Uh :c, Necesitas administrador para remover dinero a algien.");
  ...
  let tuObjeto = await moneydb.get(`${message.guild.id}.${usuario.user.id}`);
        ...
        tuObjeto.cash -= cantidad
         tuObjeto.save();
    return message.channel.send(`Removidos <:coin:716941705793765377>${cantidad} del efectivo de **${usuario.user.tag}**`)
    }
    ...
    return  message.channel.send('No puedes quitarle mas dinero del banco del que tiene, pero el banco de **'+usuario.user.tag+'** ha sido puesto en 0')
      ...
      let tuObjeto2 = await moneydb.get(`${message.guild.id}.${usuario.user.id}`);
        ...
        tuObjeto2.bank -= cantidad
         tuObjeto2.save();
     return message.channel.send(`Removidos <:coin:716941705793765377>${args[1]} del banco de **${usuario.user.tag}**`)
    }
