...
let usuariocooldown = await cooldowns.get(`${message.guild.id}.${message.author.id}.work`);
  ...
  if(await cooldowns.get(`${message.guild.id}.${message.author.id}.work`) === undefined) { 
       ...
       let sumademoney = [30, 24, 34, 28, 33, 35, 25, 39, 32, 37, 21];
  ...
  let tuObjeto = await moneydb.get(`${message.guild.id}.${message.author.id}`);
        ...
        tuObjeto.cash += cantidad
         tuObjeto.save();
  let timeoutwork = Date.now() + 300000
  ...
  let object2 = cooldowns.create(`${message.guild.id}.${message.author.id}`, {
   ...
   return message.channel.send(new Discord.RichEmbed()
                        .setAuthor(message.author.tag, message.author.displayAvatarURL)
                         .setColor(0x05d400)
                         .setTimestamp()
                         .setDescription(`Gracias a tu gran esfuerzo trabajando has ganado <:coin:716941705793765377>`+sumademoney[result222])
                        );
 ...
 let tuObjeto2 = await cooldowns.get(`${message.guild.id}.${message.author.id}`);
        tuObjeto2.work = Date.now() + 300000
         tuObjeto2.save();
  ...
  languages: {
    ...
    return message.channel.send('Puedes trabajar de nuevo en **'+timeremaing2+'** ya que ahora estás cansado. ;)');
  }
