...
exports.run = async(client, message, args, Discord) =>{
 ...
 if(!usuario) return message.channel.send("Escribe la id o menciona al usuario al que le quieres robarle dinero >:)");
  if(usuario.user.id === message.author.id) return message.channel.send("Escribe la id o menciona al usuario al que le quieres robarle dinero >:)");
  ...
  let tuObjeto = await moneydb.get(`${message.guild.id}.${usuario.user.id}`);
   ...
   let tuObjeto1 = await moneydb.get(`${message.guild.id}.${message.author.id}`);
        ...
        tuObjeto1.cash -= cantidad
         tuObjeto1.save();
    return message.channel.send(`Le has pagado <:coin:716941705793765377>${cantidad} a **${usuario.user.tag}**`)
